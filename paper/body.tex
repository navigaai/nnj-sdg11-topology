...
disruption and a milder \emph{hazard} scenario, the coarse $5\times3$ and full
...
grid-sensitive effect throughout.
...
ranking is unchanged. A milder \emph{hazard} scenario (removing edges near the
lowest-elevation $10\%$ of nodes, from AWS Terrain Tiles DEMs) is topography-
dependent: substantial only in the two flattest cities (Amsterdam and the Phoenix
basin, mean AUC $0.09$ and $0.31$) and near-zero elsewhere, so it barely moves the
city-wide $H_0$ topology and does not resolve a morphology signal (only orientation
entropy remains significant, $+0.10$, $p=0.01$). We read the hazard result as a
limitation of the mild, $\rho$-capped low-elevation proxy, and it motivates a
stronger flood model that removes \emph{all} inundated edges.
...
GHS-POP grid (R2023A, $1$~km) for the population-weighted validation; and elevation
data from the public AWS Terrain Tiles service. No new data were collected. The derived
