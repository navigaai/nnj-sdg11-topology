...
(Fig.~\ref{fig:morphology}). Because the raw coefficients sit on different scales, we
read magnitude from the standardised $\beta$ and the $10$th--$90$th percentile effect
...
appropriate small-cluster correction for our eight clusters (the eight cities)---
...
the spatial-error and spatial-lag models ($|z|\approx10$). We therefore treat circuity
and mean street length as the robust pair and orientation entropy as a real but
...
grid ($8\times10$). We report the standardised coefficient (std.\ $\beta$) and the
...
\emph{proposed proxy} for access resilience, and this is the study's central
limitation. What we establish rigorously is an \emph{association} between morphology
...
that the indicator is itself a valid measure of resilience, which we do not fully
demonstrate. We offer evidence of two kinds. Its
...
inclusive green-space definition and the constant $4.8$~km/h walking speed. We tested
both. The strict, accessibility-conservative green-space subset leaves the headline
...
the coefficient signs, significances or the city ranking, which we confirm
...
entropy is associated with less resilience and is strong in the spatial models but
less certain across cities. Two boundaries frame the contribution. The relationship
is an \emph{association}, not a causal effect: the design is cross-sectional and
observational. And it is an association with our \emph{proposed} topological
