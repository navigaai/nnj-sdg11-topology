...
\emph{estimated} spatial models (Table~\ref{tab:spatial}). The three headline
descriptors keep their sign and stay strongly significant ($|z|\ge5$) under both a
spatial-error and a spatial-lag specification, even though both absorb a large
amount of spatial dependence ($\hat\lambda=0.55$, $\hat\rho=0.53$); we return to the
implications in the Limitations.
...
significant---more green area and greater centre distance predict greater
resilience, more major-road edges less---so they are genuine correlates of AUC, yet
...
protects access to nature under stress: the descriptors that predict resilience
...
could drive resilience. We control for the four that our data support---green-space
...
for a disconnection-aware access measure; a full head-to-head against an established
measure (e.g.\ the degradation of a population-weighted two-step floating catchment
score, or observed post-disruption service loss), and an \emph{extended-persistence}
formulation that keeps disconnected components in the diagram, are the two most
important next steps. We frame the metric as a candidate indicator until they are
done. Two further caveats:
