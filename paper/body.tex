...
\paragraph{Benchmark against non-topological baselines.} A topological metric should
earn its complexity. We therefore compare it, per district, against two
non-topological baselines computed under the same random disruption but without
persistent homology: a \emph{distance-based} degradation (the increase in mean
walk-time to green space) and a \emph{connectivity} degradation (the increase in the
fraction of nodes left unable to reach any green space). The latter is the natural
non-persistent counterpart, since $H_0$ of a sublevel filtration is exactly
single-linkage connectivity. Two findings follow, one reassuring and one cautionary.
First, the topological AUC is \emph{not} redundant with either baseline: controlling
for both (and city fixed effects), the three headline morphology descriptors remain
significant ($p<10^{-6}$), so the metric carries configurational information the
simple measures miss. Second, its \emph{convergent} validity is weak---Spearman
$\rho=+0.07$ with distance degradation (right sign, negligible magnitude) and, more
tellingly, $\rho=-0.29$ with connectivity degradation, the \emph{wrong} sign. This
negative correlation is the finite-diagram effect seen for Phoenix operating
district-wide: when severe damage disconnects nodes, they leave the finite diagram,
so the Wasserstein distance can fall precisely when access loss is worst. We read
this as genuine evidence that the metric captures a distinct, configuration-sensitive
notion of resilience, but that it is \emph{not} a validated substitute for a
disconnection-aware access measure---a bound we make explicit in the Limitations and
a target for the extended-persistence variant we propose there.
...
omitted-variable controls. On \emph{criterion} validity the evidence is more mixed
and we report it plainly. Benchmarked against two non-topological baselines
(distance and connectivity degradation, above), the metric is not redundant---the
morphology signal survives controlling for them---but its convergent correlation
with a simple distance-based ground truth is weak, and with connectivity degradation
it even carries the wrong sign because disconnected nodes leave the finite diagram.
The metric is therefore best understood as capturing a distinct,
configuration-sensitive aspect of resilience rather than as a validated substitute
for a disconnection-aware access measure. To confirm that the wrong-sign result is
indeed the disconnection mechanism, we ran a diagnostic \emph{finite-cap} variant
that keeps unreachable nodes in the diagram at a large finite birth
($\tau=60$~min) instead of dropping them at infinity. The cap flips the correlation
with connectivity degradation from negative to (weakly) positive
($-0.10\to+0.02$ on a two-city reduced-grid check), confirming the diagnosis---but
it simultaneously erodes the correlation with graded distance degradation
($+0.41\to+0.06$), because the many capped classes swamp the graded signal. A naive
cap thus trades one artefact for another; a principled \emph{extended-persistence}
formulation---which records disconnection as a distinct essential feature rather
than collapsing it to a constant---is the right fix and the most important
methodological next step, alongside a full head-to-head against an established
measure (e.g.\ the degradation of a population-weighted two-step floating catchment
score, or observed post-disruption service loss). We frame the metric as a
candidate indicator until these are done. Two further caveats:
