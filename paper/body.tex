...
the district typology (Fig.~\ref{fig:resilience}): city-level
$\mathrm{AUC}$ is lowest---most resilient---for Barcelona ($620$) and Amsterdam
($898$), and highest for İstanbul ($2440$) and Bogotá ($2828$), with Phoenix in
between ($2053$). The compact, walkable cities keep their access topology intact
...
agree on every coefficient sign. The mathematics of persistence turns a static
SDG~11.7 indicator into a dynamic, form-aware one. Future work will strengthen the
hazard model to a full flood-zone removal, add a network-metric filtration, and
test resolution sensitivity, toward a resilience-aware accessibility standard for
sustainable cities.
