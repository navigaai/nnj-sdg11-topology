...
omitted-variable controls. What we do \emph{not} yet provide is external
(\emph{criterion}) validation: a head-to-head benchmark against an established
accessibility or resilience measure---for example the degradation of a
population-weighted two-step floating catchment score, or observed service loss
after a real disruption---to show the topological signal adds information beyond a
simpler distance-based one. That comparison is the most important next step and we
frame the metric as a candidate indicator until it is done. Two further caveats:
