...
\emph{opposing} signature---most strikingly, more circuitous districts are graded-more-
...
channels, as done here, and the metric's ``resilience'' should be read as the former,
not as global accessibility resilience.
...
present data qualify. The tempting reading---that \emph{path redundancy}, not raw
...
topological summary scales with configuration granularity is exactly what an
...
check sharpens the picture. Because the AUC scales strongly with the raw size of the
...
served $H_0$ structure reorganises under diffuse loss are exactly what the metric
...
predicts urban resilience'' therefore overreaches; the defensible statement is that
street morphology is associated with topological access resilience \emph{under the
...
hold for distributed disruption and do not carry to the real clustered-flood scenario. Two boundaries frame the contribution. The relationship
