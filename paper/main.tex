...
ranking are unchanged. The finding is therefore not an artefact of the
disruption model.
...
\paragraph{Limitations.} (i) We report the random and targeted scenarios, which
agree on every sign; the hazard (low-elevation) scenario is implemented but
requires per-city digital elevation models and is left for future work. (ii)
...
SDG~11.7 indicator into a dynamic, form-aware one. Future work will add the
hazard scenario, a network-metric filtration, and resolution-sensitivity tests,
toward a resilience-aware accessibility standard for sustainable cities.
