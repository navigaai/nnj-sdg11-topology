...
\includegraphics[width=0.72\linewidth]{figures/fig6_morphology.png}
...
(Fig.~\ref{fig:resilience}): $\Drho = (0,\,279.5,\,1276.1,\,1766.1)$ at
$\rho=(0,0.1,0.3,0.5)$, with $\mathrm{AUC}=947.5$ and $\rhostar=0.30$, confirming
that the metric responds meaningfully to disruption on real data.

\begin{figure}[t]\centering
\includegraphics[width=0.6\linewidth]{figures/fig5_resilience.png}
\caption{City-level resilience curve $\Drho$ for Amsterdam (random-disruption
scenario). The dotted vertical line marks the critical transition $\rhostar$.}
...
persistence-thresholded $H_0$ metric and a reduced disruption grid; magnitudes are
therefore denoised estimates, though ranking, signs and $\rhostar$ are stable.
(iv) City-level degradation curves (Fig.~5) are so far computed for Amsterdam
only. (v) OSM completeness varies by city, and the network-distance
Vietoris--Rips variant of the filtration is left to future work. None of these undermines the core finding: at the scale
